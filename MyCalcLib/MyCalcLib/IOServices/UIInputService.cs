		...
		private void GetExpressionMembers()
		{
			try
			{
				...
				if (operationSymbol == "^")
				{
					...
					Logger.Info("Set operation pocer indicator for operation {0}.",operationSymbol.ToString());
				}
				else if (operationSymbol.ToString() == "√")
				{
					operation = OperationType.Sqrt;
					Logger.Info("Set operation {0}.",operation.ToString());
				}
				else
				{
					operation = (OperationType)Convert.ToChar(expressionMembers[2]);
					Logger.Info("Set operstion {0}",operation.ToString());
			...
			catch (Exception ex)
			{
				...
				throw new ArgumentException(nameof(GetExpressionMembers), "Can execute this operation.");
			}
